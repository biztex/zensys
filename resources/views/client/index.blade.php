...
<div class="row mb-2">
  ...
  <div class="col-sm-6 mt-3">
    <a href="{{config('app.url')}}client/create" class="btn btn-primary float-right">新規追加</a>　
  </div>
</div>
<div id="result"></div>
<div class="row mb-2">
  <div class="col-sm-12 mt-4">
    <form method="post" name="form" action="{{config('app.url')}}client/destroy-selected">
      @csrf
...
new gridjs.Grid ({
  ...
  columns: [
    ...
    { 
      ...
      sort: false,
        formatter: (_, row) => gridjs.html(`
          <div class="row">
            <a href="{{config('app.url')}}client/edit/${row.cells[1].data}" class="btn btn-warning btn-sm">編集</a>　
            <form method="post" name="form" action="{{config('app.url')}}client/destroy/${row.cells[1].data}">
              @csrf
  ...
  server: {
    url: '{{config('app.url')}}client/json',
