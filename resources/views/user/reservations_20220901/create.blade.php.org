...
<small>・{{ date('Y年m月d日', strtotime($date)) }} </small><br />
            </div>
        </div>
...
if($stock_price_types){
    foreach ($stock_price_types as $i => $stock_price_type) {
        ...
        if ($i == 0) {
            echo '<span><input type="text" name="type' . $stock_price_type->price_type_number . '_number" class="number" min="0" max="999" value="1"><span class="number-text"> 人</span></span></p>';
        } else {
            echo '<span><input type="text" name="type' . $stock_price_type->price_type_number . '_number" class="number" min="0" max="999" value="0"><span class="number-text"> 人</span></span></p>';
        }
    }
}else{
...
@endif
                ...
                <tr>
                  <th>目的地</th><td><strong class="place-placeholder"></strong><br />{{ $plan->place_name }}
...
アクセス：{{ $plan->place_access }}
        ...
        <div class="content"> 
            <table>
                <tr>
                  <th>キャンセル規定</th>
                  <td>参加日の10日〜8日前 　：　旅行代金の20%<br /><hr />参加日の1週間前〜前日　：　旅行代金の30%<br /><hr />参加日の前日キャンセル　：　旅行代金の40%<br /><hr />参加日の当日キャンセル　：　旅行代金の50%<br /><hr />参加日の無連絡キャンセル：　旅行代金の100%</td>
                </tr>
            </table>
        </div>
